...
- Persistência em **SQLite** via EF Core (`trilha.db`, criado/migrado automaticamente ao iniciar).

## O que falta (próximos passos)

- Recuperação de senha (endpoint `esqueci-minha-senha` ainda não existe).
...
- Trocar a chave JWT em `appsettings.json` por um segredo de produção (via variável de ambiente ou `dotnet user-secrets`) antes de qualquer deploy.
...
```bash
dotnet run --project TrilhaDaMultiplicacaoAPI
```

O banco SQLite é criado e migrado automaticamente no primeiro start. Em desenvolvimento, o OpenAPI fica disponível em `/openapi/v1.json`.

## Integração com o app desktop

O app desktop já isola a persistência atrás de `IProgressoRepository`. O próximo passo é criar uma implementação dessa interface que consuma esta API via `HttpClient`, substituindo o registro do `SessionService` mockado em `App.axaml.cs` — sem precisar alterar nenhuma tela.
